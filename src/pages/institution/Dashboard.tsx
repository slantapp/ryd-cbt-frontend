...
export default function Dashboard() {
    ...
    try {
      if (account?.role === 'SUPER_ADMIN') {
        ...
        if (response?.data) {
          setTeacherStats({
            ...
            stats: response.data.stats || { classCount: 0, sessionCount: 0 },
          });
        } else {
      ...
      if (account?.role === 'TEACHER') {
        setTeacherStats({
          ...
          sessions: [],
          school: undefined,
          stats: { classCount: 0, sessionCount: 0 },
        });
      }
    } finally {
      setLoading(false);
  ...
  const renderTeacherDashboard = () => {
    ...
    const sessions = teacherStats?.sessions ?? [];
    const school = teacherStats?.school;
    const stats = teacherStats?.stats ?? { classCount: 0, sessionCount: 0 };
        ...
        {sessions.length > 0 && (
          <div className="card">
            <div className="flex justify-between items-center mb-4">
              ...
              <Link to="/sessions" className="text-primary text-sm font-medium hover:underline">
                View All →
              </Link>
            </div>
            <div className="space-y-3">
              {sessions.slice(0, 5).map((session) => (
                <div key={session.id} className="p-4 border rounded-lg hover:border-primary transition-colors">
                    ...
                    <div className="flex-1">
                      ...
                      {session.classAssignments && session.classAssignments.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-2">
                          {session.classAssignments.slice(0, 3).map((ca: any) => (
                            <span
                              key={ca.classroom?.id}
                          ...
                          {session.classAssignments.length > 3 && (
                            <span className="px-2 py-1 text-gray-500 text-xs">
                              +{session.classAssignments.length - 3} more
                            </span>
                          )}
                        </div>
                      )}
